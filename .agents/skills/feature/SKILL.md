...
For this paired flow the Codex host must be `gpt-6-astra`; the helper launches Fable as the peer with `--host astra`. If this session uses a different model, explain that the owner should run `/feature` in Claude Code (which selects Fable) or select Astra before `$feature`. Do not label another model Astra or silently run two copies of Astra. Read the canonical skill's `references/pair-planning.md` for the exact protocol.
